...
*Setup instructions for a gitian build of FairPay using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the FairPay
...
- [Building FairPay](#building-fairpay)
...
Clone the git repositories for fairpay and gitian and then checkout the fairpay version that you want to build.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/fairpay-project/fairpay.git
cd fairpay
...
Follow the instructions in [doc/release-process.md](release-process.md) in the fairpay repository
...
To build FairPay (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the fairpay repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/fairpay/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/fairpay-project/fairpay
...
For example:
```bash
URL=https://github.com/user/fairpay.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit fairpay=${COMMIT} --url fairpay=${URL} ../fairpay/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit fairpay=${COMMIT} --url fairpay=${URL} ../fairpay/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit fairpay=${COMMIT} --url fairpay=${URL} ../fairpay/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/fairpay-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/fairpay-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/fairpay-build.assert
```
...
[fairpay/gitian.sigs](https://github.com/fairpay-project/gitian.sigs.ltc/) repository, or if that's not possible create a pull
request. 
